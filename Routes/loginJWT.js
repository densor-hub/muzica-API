    ...
    try {
      if (
        ...
        let UserExists = await db
          .collection("users")
          .findOne({ username: removeWhiteSpaces(req.body.username) });

        if (UserExists != null) {
          let userIsSuspended = await db
              ...
              db.collection("users")
                .replaceOne(
                  { _id: UserExists._id },
                  {
                    ...
                    stagename: UserExists.stagename,
                    image: UserExists?.image?.buffer,
                    refresher: refreshToken,
                    email: UserExists?.email,
                    websiteCreated: UserExists?.websiteCreated,
                    refresher: refreshToken,
                  }
                )
                .then((noError) => {
