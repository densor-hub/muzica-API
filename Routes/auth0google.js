...
router?.route("/")?.post(async (req, res) => {
  if (
    ...
    let UserExists = await db
      .collection("users")
      ?.findOne({ googleId: user?.sub?.split("|")[1] });
    ...
    if (UserExists !== null && UserExists !== undefined) {
      ...
      UserExists = await db
        .collection("users")
        ?.findOne({ email: String(user?.email) });
            ...
            if (results?.modifiedCount > 0) {
              res
                ...
                .json({
                  ...
                  stagename: `${UserExists.stagename}`,
                  profilePicture: UserExists.profilePicture,
                  stagenameInUrl: create_Username_url(UserExists?.stagename),
                  websiteCreated: UserExists?.websiteCreated,
                  provider: "google",
                });
            } else {
          ...
          .then((results) => {
            ...
            res
              ...
              .json({
                ...
                stagename: `${user.name}`,
                profilePicture: user.picture,
                stagenameInUrl: create_Username_url(user?.name),
                websiteCreated: "",
                provider: "google",
              });
          });
