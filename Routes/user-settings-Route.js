...
router.route('/')
.get(async(req, res)=>{
    if(!(req?.cookies && req?.session?.cookie?.httpOnly && req?.session.passport?.user)){
        res.sendStatus(401);
    }
    else{
        try {
                    ...
                    sharp(filePath).resize(200,200)?.toFile(`${path?.join(__dirname,'../uploads')}/${identifier}-cpudp.${req.files.file.mimetype.split('/')[1]}`).then((results)=>{
                        if(fileSystem?.existsSync(filePath)){
                            fileSystem?.unlink(filePath, ()=>{
                                console.log('FILE EXISTED, HENCE DELETED')
                            })
                        }
