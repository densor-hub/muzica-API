    ...
    if (!(String(req?.method) === String('GET'))) {
        ...
        try {
            ...
            if ((req?.get('origin') === undefined) || (req?.get('origin') === null)) {
                ...
                if (requestURL.split('?').length === 0 || !(new URL(requestURL)?.searchParams?.has('id') && new URL(requestURL)?.searchParams?.has("a"))) {
                    next();
                } else {

                    if (new URL(requestURL)?.searchParams.has("id") && new URL(requestURL)?.searchParams.has("a")) {
                        let id = new URL(requestURL)?.searchParams.get("id");
                        let artiste = new URL(requestURL)?.searchParams?.get('a');
