    ...
    try {
      ...
      if (isValidUser !== null && isValidUser !== undefined) {
        if (
          ...
          req?._parsedUrl?.pathname?.split(":")[1]?.length !== 24
        ) {
          next();
        } else {
          querry = req?._parsedUrl?.pathname?.split(":")[0]?.split("-")[1];
          selectedItem = await db
            .collection(`${querry?.toLowerCase()}`)
            .findOne({
              _id: ObjectId(req?._parsedUrl?.pathname?.split(":")[1]),
            });
        }
        ...
        if (
          ...
          if (
            ...
            if (selectedItem !== null && selectedItem !== undefined) {
              if (querry === "audios") {
                let convertCoverArtToBuffer = {
                  ...selectedItem,
                  image: selectedItem.image.buffer,
                };
                res?.status(200)?.json({ results: convertCoverArtToBuffer });
              } else {
            ...
            if (!req?.body) {
              ...
              if (querry === "audios") {
                if (
                  ...
                  !dateValidator.isValidDate(req?.body?.datereleased) ||
                  (req.body.applemusic &&
                    !(
                ...
                if (
                  ...
                  !req?.body?.link ||
                  !req?.body?.dateReleased ||
                  !new Date(req?.body?.dateReleased) ||
                  !dateValidator.isValidDate(req?.body?.dateReleased) ||
                  !(
                    req.body.link &&
                    req?.body?.link?.startsWith("https://") &&
                    (new URL(req?.body?.link?.trim())?.origin
                      ?.toLowerCase()
                      .endsWith("youtube"?.toLowerCase() + ".com") ||
                      new URL(req?.body?.link)?.origin.endsWith(
                        (
                          "youtube"?.toLowerCase()?.slice(0, 5) +
                          "." +
                          "youtube"?.trim().slice(5, 8)
            ...
            if (selectedItem !== null && selectedItem !== undefined) {
              if (querry === "audios" || querry === "images") {
                db.collection(querry)
                  .deleteOne({ _id: selectedItem?._id })
                  .then((results) => {
