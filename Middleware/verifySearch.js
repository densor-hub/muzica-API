  ...
  if (!(String(req?.method) === String("POST"))) {
    ...
    try {
      ...
      if (req?.get("origin") !== undefined && req?.get("origin") !== null) {
        if (
          requestURL.split("?").length === 0 ||
          !(
            new URL(requestURL)?.searchParams?.has("id") &&
            new URL(requestURL)?.searchParams?.has("artiste")
          )
        ) {
          next();
        } else {
          if (
            ...
            if (
              (id?.length === 24 && !id?.endsWith("/")) ||
              (id?.length === 25 && id?.endsWith("/"))
            ) {
              ...
              if (
                isValidUserWebsite === null ||
                isValidUserWebsite === undefined
              ) {
                res.sendStatus(404);
              } else {
                let requestedData = req?.url
                  ?.split("?")[0]
                  .split("/webpage/")[1];
                //get all posts from Database

                let unfilteredItems = await db
                ...
                unfilteredItems.forEach((element) => {
                  if (
                    String(element?.title) === String(req?.body?.searchFor) ||
                    String(element?.title)?.includes(
                      String(req?.body?.searchFor)
                    )
                  ) {
