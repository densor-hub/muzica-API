...
const equal_To_Or_Bigger_Than_Toadys_Date=(date)=>{
    if(date ===null || date === undefined || date?.split('-')?.length!==3){
        return false
    }
    else{
...
const isValidDate =(untrimedDate)=>{
    const date = String(untrimedDate).trim();
    let dateYear = date.split('-')[0];
...
module.exports ={durationValidator, equal_To_Or_Bigger_Than_Toadys_Date, isValidDate};